...
// SLAVE_DEV_FW_LOAD_T test_fw; 

// test_fw.m4_fw_size = ;
// test_fw.m4_fw_dest_addr= ;  
/******* ML Processor Specific Defines */
...
void set_gpio(int gpio)
{
    ...
    if (gpio <=7)
    {
        ...
        {
            printf("\nGPIO 0 set\n");
            faddress = 1<<0;         // 128-GPIO7 64-RLED, 32-GLED, 16-BLED    // MISC_IO_OUTPUT  ALL RGB LED
            printf("write\n");
            QLSPI_Write_S3_Mem(0x40005104,(unsigned char *)&faddress,4);
            vTaskDelay(50 / portTICK_PERIOD_MS);

            tester = 0;
            printf("Read 0x40005104 \n");
        ...
        {
            printf("\nGPIO 1 set\n");
            faddress = 1<<1;         // 128-GPIO7 64-RLED, 32-GLED, 16-BLED    // MISC_IO_OUTPUT  ALL RGB LED
            printf("write\n");
            QLSPI_Write_S3_Mem(0x40005104,(unsigned char *)&faddress,4);
            vTaskDelay(50 / portTICK_PERIOD_MS);
        ...
        {
            printf("\nGPIO 2 set\n");
            faddress = 1<<2;         // 128-GPIO7 64-RLED, 32-GLED, 16-BLED    // MISC_IO_OUTPUT  ALL RGB LED
            printf("write\n");
            QLSPI_Write_S3_Mem(0x40005104,(unsigned char *)&faddress,4);
            vTaskDelay(50 / portTICK_PERIOD_MS);
        ...
        {
            printf("\nGPIO 3 set\n");
            faddress = 1<<3;         // 128-GPIO7 64-RLED, 32-GLED, 16-BLED    // MISC_IO_OUTPUT  ALL RGB LED
            printf("write\n");
            QLSPI_Write_S3_Mem(0x40005104,(unsigned char *)&faddress,4);
            vTaskDelay(50 / portTICK_PERIOD_MS);
        ...
        {
            printf("\nGPIO 6 set\n");
            faddress = 1<<6;         // 128-GPIO7 64-RLED, 32-GLED, 16-BLED    // MISC_IO_OUTPUT  ALL RGB LED
            printf("write\n");
            QLSPI_Write_S3_Mem(0x40005104,(unsigned char *)&faddress,4);
            vTaskDelay(50 / portTICK_PERIOD_MS);
        ...
        {
            printf("\nGPIO 7 set\n");
            faddress = 1<<7;         // 128-GPIO7 64-RLED, 32-GLED, 16-BLED    // MISC_IO_OUTPUT  ALL RGB LED
            printf("write\n");
            QLSPI_Write_S3_Mem(0x40005104,(unsigned char *)&faddress,4);
            vTaskDelay(50 / portTICK_PERIOD_MS);
...
void clear_all_gpio(void)
{
    printf("clear all gpio function\n");
    faddress = 0 ;         // 128-GPIO7 64-RLED, 32-GLED, 16-BLED    // MISC_IO_OUTPUT  ALL RGB LED
    printf("write\n");
    QLSPI_Write_S3_Mem(0x40005104,(unsigned char *)&faddress,4);
    vTaskDelay(50 / portTICK_PERIOD_MS);
...
void app_main(void)
{
    ...
    // int errorval = 0;
    // flash_img_entry immg1;  //,img_fpga,img_m4;
    ...
    //FFE Read & Load
    // qlfwloader2_ffe_load(0);
    // vTaskDelay(5 / portTICK_PERIOD_MS);

    // immg1.size = sizeof(M4_FILE_RAW_DATA);
    // immg1.start_address = 0;
    // for (faddress = 0;faddress < immg1.size;faddress+=QLSPI_CHUNK_SIZE)
    // {
    //     if ((faddress+QLSPI_CHUNK_SIZE) < immg1.size)
    //     {
    //         memset(texty,0x33,sizeof(texty));
    //         //spif_test(0x10,immg1.start_address+faddress,QLSPI_CHUNK_SIZE,texty);
    //         memcpy(&texty[3],&M4_FILE_RAW_DATA[faddress],QLSPI_CHUNK_SIZE);
    //         QLSPI_Write_S3_Mem(0x20000000+faddress,(unsigned char *)(&texty[3]),QLSPI_CHUNK_SIZE);
    //         memset(texty,0x22,sizeof(texty));
    //         QLSPI_Read_S3_Mem(0x20000000+faddress,(unsigned char *)(&texty[3]),QLSPI_CHUNK_SIZE);
    //         for (immg1.start_address = 0; immg1.start_address < QLSPI_CHUNK_SIZE;immg1.start_address++)
    //             if (texty[3+immg1.start_address] !=  M4_FILE_RAW_DATA[faddress+immg1.start_address])
    //             errorval++;
    //     }
    //     else
    //     {
    //         memset(texty,0x33,sizeof(texty));
    //         //spif_test(0x10,immg1.start_address+faddress,(immg1.size-faddress),texty);
    //         memcpy(&texty[3],&M4_FILE_RAW_DATA[faddress],(immg1.size-faddress));
    //         QLSPI_Write_S3_Mem(0x20000000+faddress,(unsigned char *)(&texty[3]),(immg1.size-faddress));
    //         memset(texty,0x22,sizeof(texty));
    //         QLSPI_Read_S3_Mem(0x20000000+faddress,(unsigned char *)(&texty[3]),(immg1.size-faddress));
    //         for (immg1.start_address = 0; immg1.start_address < (immg1.size-faddress);immg1.start_address++)
    //             if (texty[3+immg1.start_address] !=  M4_FILE_RAW_DATA[faddress+immg1.start_address])
    //             errorval++;
    //     }
    // }
